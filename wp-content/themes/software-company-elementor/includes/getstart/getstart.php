...
// Changelog
if ( ! defined( 'SOFTWARE_COMPANY_ELEMENTOR_CHANGELOG_URL' ) ) {
    define( 'SOFTWARE_COMPANY_ELEMENTOR_CHANGELOG_URL', get_template_directory() . '/readme.txt' );
}

function software_company_elementor_changelog_screen() {	
	global $wp_filesystem;
	$changelog_file = apply_filters( 'software_company_elementor_changelog_file', SOFTWARE_COMPANY_ELEMENTOR_CHANGELOG_URL );
	if ( $changelog_file && is_readable( $changelog_file ) ) {
		WP_Filesystem();
		$changelog = $wp_filesystem->get_contents( $changelog_file );
		$changelog_list = software_company_elementor_parse_changelog( $changelog );
		echo wp_kses_post( $changelog_list );
	}
}

function software_company_elementor_parse_changelog( $content ) {
...
?>
    ...
    <div class="inner-cont">
	    <div class="tab-sec">
	    	<div class="tab">
				<button class="tablinks" onclick="software_company_elementor_open_tab(event, 'wpelemento_importer_editor')"><?php esc_html_e( 'Setup With Elementor', 'software-company-elementor' ); ?></button>
			...
			<div id="wpelemento_importer_editor" class="tabcontent open">
				<?php if(!class_exists('WPElemento_Importer_ThemeWhizzie')){
					$plugin_ins = Software_Company_Elementor_Plugin_Activation_WPElemento_Importer::get_instance();
					$software_company_elementor_actions = $plugin_ins->recommended_actions;
					?>
					<div class="software-company-elementor-recommended-plugins ">
							<div class="software-company-elementor-action-list">
								<?php if ($software_company_elementor_actions): foreach ($software_company_elementor_actions as $key => $software_company_elementor_actionValue): ?>
										<div class="software-company-elementor-action" id="<?php echo esc_attr($software_company_elementor_actionValue['id']);?>">
											<div class="action-inner plugin-activation-redirect">
												<h3 class="action-title"><?php echo esc_html($software_company_elementor_actionValue['title']); ?></h3>
												<div class="action-desc"><?php echo esc_html($software_company_elementor_actionValue['desc']); ?></div>
												<?php echo wp_kses_post($software_company_elementor_actionValue['link']); ?>
											</div>
										</div>
									<?php endforeach;
								endif; ?>
				...
				<?php }else{ ?>
					<div class="tab-outer-box">
						<h3><?php esc_html_e('Welcome to WPElemento Theme!', 'software-company-elementor'); ?></h3>
						<p><?php esc_html_e('Click on the quick start button to import the demo.', 'software-company-elementor'); ?></p>
						<div class="info-link">
							<a  href="<?php echo esc_url( admin_url('admin.php?page=wpelementoimporter-wizard') ); ?>"><?php esc_html_e('Quick Start', 'software-company-elementor'); ?></a>
						</div>
					</div>
				<?php } ?>
			</div>
			...
			<div id="changelog_cont" class="tabcontent">
				<div class="tab-outer-box">
					<?php software_company_elementor_changelog_screen(); ?>
				</div>
			</div>
		...
		<div class="inner-side-content">
			...
			<div class="tab-outer-box">
				...
				<div class="iner-sidebar-pro-btn">
					<span class="premium-btn"><a href="<?php echo esc_url( SOFTWARE_COMPANY_ELEMENTOR_BUY_NOW ); ?>" target="_blank"><?php esc_html_e('Buy Premium', 'software-company-elementor'); ?></a>
					</span>
					...
					<span class="doc-btn"><a href="<?php echo esc_url( SOFTWARE_COMPANY_ELEMENTOR_THEME_BUNDLE ); ?>" target="_blank"><?php esc_html_e('Theme Bundle', 'software-company-elementor'); ?></a>
					</span>
				</div>
				<hr>
				<div class="premium-coupon">
					...
					<div class="coupon-box">
						<h3><?php esc_html_e('Use Coupon Code', 'software-company-elementor'); ?></h3>
						<a class="coupon-btn" href="<?php echo esc_url( SOFTWARE_COMPANY_ELEMENTOR_BUY_NOW ); ?>" target="_blank"><?php esc_html_e('UPGRADE NOW', 'software-company-elementor'); ?></a>
						<div class="coupon-container">
							<h3><?php esc_html_e( 'elemento20', 'software-company-elementor' ); ?></h3>
						</div>
					</div>
				</div>
			</div>	
